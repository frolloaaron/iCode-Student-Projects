...
public partial class Platform_MainPage : ContentPage
{
    ...
    private Player_P Create_User()
    {
        Image userIcon = new Image { Source = "cartoonpenguin.png", ZIndex = 1 };
        user = new Player_P(userIcon, 4, 2);
        return user;
    }
...
public class Player_P
{
    ...
    public int row;
    public int col;
    public Player_P(Image i, int r, int c)
    {
        image = i;
        row = r;
        col = c;
    ...
    public bool Jump(Grid g, Platform[] list, bool con, Platform_MainPage page)
    {
        ...
        else
        {
            ...
            if (this.col == plat.col && this.row == plat.row)
            {
                page.score++;
